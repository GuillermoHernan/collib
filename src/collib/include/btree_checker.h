...
public:
	...
	BTreeCoreChecker(const CoreType& core) : m_core(core) {}
...
private:
	...
	static std::string indent(unsigned level)
	{
	...
	)
	{
		...
		if (isLeaf)
		{
			...
			check(
				!(leaf.key(0) < minKey)
				, "Leaf key (", keyToString(leaf.key(0))
				, ") below minimum bound (", keyToString(minKey), ")"
			);
			check(
				!(maxKey < leaf.key(leaf.count() - 1))
				, "Leaf key (", keyToString(leaf.key(leaf.count() - 1))
				, ") above or equal maximum bound (", keyToString(maxKey), ")"
			);
	...
	bool checkLeafChain()
	{
		...
		while (current)
		{
			...
			if (next)
			{
				...
				ok &= check(current->key(current->count() - 1) < next->key(0),
...
public:
	...
	BTreeChecker(const MapType& map) : m_core(map.m_core) {}
