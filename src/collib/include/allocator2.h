...
struct IAllocator
{
     ...
     */
    virtual SAllocResult alloc(byte_size bytes, align a) = 0;

    /* Will try to expand a given allocated block WITHOUT MOVING it.
     * Returns the new size, which COULD BE lower than the requested size, but at least the original
     * size. The calling code must check the return value and decide if the new size is enough. This
     * interface does not support a realloc operation which moves data. That must be handled by the
     * calling code.
     */
    virtual byte_size tryExpand(byte_size bytes, void*) = 0;
...
{
    const SAllocResult r = alloc.alloc(sizeof(T), align::of<T>());
